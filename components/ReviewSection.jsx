...
const ReviewSection = ({ productId }) => {
    ...
    const checkCanReview = useCallback(async () => {
        ...
        try {
            const res = await axios.get(`/api/review/canReview?productId=${productId}`);
    ...
    const renderStars = (rating) => {
        ...
        for (let i = 1; i <= 5; i++) {
            if (i <= rating) {
                starts.push(<Star key={i} className="w-5 h-5 text-yellow-400 fill-yellow-400" />);
            } else if (i - 0.5 <= rating) {
                starts.push(<StarHalf key={i} className="w-5 h-5 text-yellow-400 fill-yellow-400" />);
            } else {
                        ...
                        <div className="mb-4 md:mb-6">
                            ...
                            <div className="flex space-x-1">
                                {[1, 2, 3, 4, 5].map((star) => (
                                    <Star
                                        key={star}
                                        className={`cursor-pointer w-6 h-6 md:w-8 md:h-8 transition-all duration-200 ${star <= userReview.rating
