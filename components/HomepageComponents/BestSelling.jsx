...
const BestSellingWatches = () => {
    ...
    const prevWatch = () => setCurrentIndex((prevIndex) => (prevIndex - 1 + watches.length) % watches.length);
                    ...
                    <div className="flex items-center sm:h-[27rem] h-[35rem] justify-center overflow-hidden">
                        {Array.from({ length: 5 }).map((_, offset) => {
                            ...
                            return (
                                <div
                                    key={watch.id}
                                    className={`transition-all duration-300 flex-shrink-0 w-full md:w-1/3 lg:w-1/5 px-2 ${offset === 2 ? "scale-105 z-10" : "scale-95 opacity-75"
                                        }`}
                                >
                                    <Link
                                        href={watch.link}
