...
const ProfilePage = () => {
  ...
  const [profile, setProfile] = useState({name: "", email: "", admin: false, image: ""});
...
const handleSave = async ()=> {
  ...
  try {
    ...
    if(res.status === 200){
      await update({
        ...session, user: {...session?.user, name: profile.name, email: profile.email},
      });
      setProfile((prevProfile) => ({
        ...prevProfile, name: profile.name, email: profile.email
      }));
