...
async function createUser(email, password, name) {
    ...
    const newUser = new User({
        email, password: hashedPassword, name, profileImage: DEFAULT_PROFILE_IMAGE
    });
        ...
        CredentialsProvider({
            ...
            async authorize(credentials) {
                ...
                try {
                    ...
                    // ensure user have profile image
                    ...
                    console.log("Error ait api/nextauth:", error);
                    return null;
                }
            },
        }),
    ...
    callbacks: {
        async signIn({ user, account }) {
            if (account.provider === "google") {
                try {
                    ...
                    user.notificaionPreferances = foundUser.notificaionPreferances;
        ...
        async jwt({ token, user, trigger, session }) {
            if (user) {
                ...
                token.notificaionPreferances = user.notificaionPreferances;
        ...
        async session({ session, token }) {
            if (session.user) {
                ...
                session.user.notificaionPreferances = token.notificaionPreferances;
